...
//			stIter = model.listStatements(null, foafKnows, janeSmith);
			stIter = model.listStatements(null, FOAF.knows, janeSmith);

			while (stIter.hasNext())
			{
				Statement st2 = stIter.next();
				Resource subj = st2.getSubject();
				
				StmtIterator stIter2 = model.listStatements(subj, VCARD.Given, (RDFNode)null);
				
				while (stIter2.hasNext())
				{
					Statement st3 = stIter2.next();
