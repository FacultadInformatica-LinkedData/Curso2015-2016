var val='';
			...
			$('button').on("click",function(){
				$('p').remove();
				$('ul').remove();
				if(val == '') $('body').append("<p>Seleccione un área válido</p>");
				else{
					var valT = $("textarea").val();
					if(valT == '') $('body').append("<p>Busca algo</p>");
					else{
							...
							$.each(obj, function(i2, obj2){
								...
								if(i2 == val2 && obj2 == valT){
									...
									if(selec == 2){
										...
										$('body').append("<p>Seleccione un área válido</p>")
									}
								}
							})
						})
			...
			function appendBody(){
				...
				if(selec == 2) $('table').append('<tr><th>NOMBRE</th><th>NOMBRE-VIA</th><th>NUM</th><th>BARRIO</th><th>DISTRITO</th><th>CODIGO-POSTAL</th></tr>')
				else if(selec == 3) $('table').append('<tr><th>NOMBRE</th><th>NOMBRE-VIA</th><th>NUM</th><th>BARRIO</th><th>DISTRITO</th><th>CODIGO-POSTAL</th></tr>')
				else if(selec == 4)$('table').append('<tr><th>NOMBRE-VIA</th><th>NUM</th></tr>')
				else if(selec == 5) $('table').append('<tr><th>NOMBRE-VIA</th><th>NUM</th></tr>')
			...
			function rellenarSelect(){
				...
				if(val == "aparcamiento"){
					...
					$('#2').empty().append('<option value="*" selected>Seleccione el campo a buscar</option>'+
						...
						'<option value="Calle">Nombre de la calle/avenida</option>');
				}
				else if(val == "acusticos"){
					selec = 4;
					$('#2').empty().append('<option value="*" selected>Seleccione el campo a buscar</option>'+
						'<option value="LOCALIZACI">Barrio</option>'+
